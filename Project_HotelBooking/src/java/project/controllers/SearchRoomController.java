...
public class SearchRoomController extends HttpServlet {
    ...
    private final Logger log4j = Logger.getLogger(SearchRoomController.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (search.isEmpty()) {
                url = ERROR;
                request.setAttribute("SEARCH_ROOM_ERROR", "Please input somethings to search !");
            } else {
                RoomDAO dao = new RoomDAO();
                List<RoomDTO> list = dao.searchByHotelName(search, 1, 3);
                if (list.isEmpty() || list == null) {
                    list = dao.searchByHotelName("", 1, 3);
                    request.setAttribute("SEARCH_ROOM_ERROR", "Not Found");
                }
            ...
            session.setAttribute("SEARCH_ROOM_VALUE", search);
            session.removeAttribute("SEARCHBYDATE");
            session.removeAttribute("ADDRESS");
            session.removeAttribute("AMOUNT");
        } catch (Exception e) {
            log4j.error("Error at SearchRoomController : ", e);
        } finally {
